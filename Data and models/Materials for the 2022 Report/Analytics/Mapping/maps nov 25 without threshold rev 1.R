...
make_map <- function(year, title) {
  ...
  ggplot() +
    ...
    theme_void() +
    theme(
      legend.position = "none",  # suppress here, legend added separately
      plot.title = element_text(size = 14, face = "bold", hjust = 0.5)
    )
}
...
# --- Create a separate legend plot (same settings as Combined Health Workforce) ---
create_legend_plot <- function() {
  dummy_data <- data.frame(
    category = factor(c(break_labels, "Not in AFRO"), 
                     levels = c(break_labels, "Not in AFRO")),
    value = 1:length(c(break_labels, "Not in AFRO"))
  )
...
# Doctors, Nurses & Midwives density maps with discrete levels
...
# Doctors, Nurses, Midwives, Pharmacists & Dentists density maps with discrete levels
